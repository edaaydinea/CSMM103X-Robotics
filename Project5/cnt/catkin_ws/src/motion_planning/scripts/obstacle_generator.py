...
class ObstacleGenerator(object):
    ...
    def helper(self):
        self.identity_caller()
        threading.Timer(20, self.helper).start()
       
    def identity_caller(self):
        if self.obs_type == "no_obstacle":
            self.no_obs()
        elif self.obs_type == "simple_obstacle":
            self.simple_obs()
        elif self.obs_type == "complex_obstacle":
            self.complex_obs()
        else:
            self.super_obs()
    ...
    def complex_obs(self):
        self.no_obs()
        self.obs_type = "complex_obstacle"
        pose_stamped = geometry_msgs.msg.PoseStamped()
        pose_stamped.header.frame_id = "world_link"
        pose_stamped.header.stamp = rospy.Time(0)
