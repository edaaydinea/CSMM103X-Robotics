...
def cartesian_control(joint_transforms, b_T_ee_current, b_T_ee_desired,
    ...
    ## 1. Compute the desired change in end-effector pose from b_T_ee_current to b_T_ee_desired
    ...
    ## 2. Convert desired change into a desired end-effector velocity
    delta_X = numpy.append(b_t_ee * 3, ROT * 1.5)

    # This is essentially a velocity controller in end-effector space.
    X_dot = 1 * delta_X

    ## 3. Numerically compute the robot Jacobian. 
    J = numpy.empty((6,0))

    # For each joint compute the mtraix that relates the velocity of that joint to the velocity of the end-effector in its own coordinate frame.
    for j in range(num_joints):
        ...
        # Revolute joints
        Vj = numpy.append(
            numpy.append(ee_R_j, numpy.dot(-ee_R_j, S), axis=1),
            numpy.append(numpy.zeros([3,3]), ee_R_j, axis=1),
            axis = 0)
        
        J = numpy.column_stack((J, Vj[:,5])) 

    ## 4. Compute the pseudo-inverse of the Jacobian. 
        
    J_pseudo_inverse = numpy.linalg.pinv(J, rcond=1e-2)
    dq = numpy.dot(J_pseudo_inverse, X_dot)

    if red_control == True:
        J_pseudo_inverse = numpy.linalg.pinv(J, rcond=0)
        dq_n = numpy.dot(
            numpy.identity(7) - numpy.dot(J_pseudo_inverse, J),
            numpy.array([q0_desired - q_current[0],0,0,0,0,0,0]))
        dq = numpy.dot(J_pseudo_inverse, X_dot) + dq_n
    #----------------------------------------------------------------------
    return dq
